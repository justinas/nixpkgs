...
let
  ...
  mkTest = { name, useNftables ? false, firewall ? true }: makeTest {
    name = "upnp_${name}";
    ...
    testScript = ''
      ...
      client2.fail("curl --connect-timeout 1 -f http://${externalRouterAddress}:9000/")
...
in
{
  ...
  # TODO after #207758 implement the following:
  /*
  nftables-filterforward = mkTest {
    name = "nftables";
    useNftables = true;
    filterForward = true;
  };
